...
func (bm *Benchmark) CSV() string {
	...
	for i, g := range groups {
		...
		for j, c := range g.Samples {
			r := []string{bm.RunID, fmt.Sprintf("%s%d", pre, j), fmt.Sprintf("%d", c.MsgBytes), fmt.Sprintf("%d", c.Rate()), fmt.Sprintf("%f", c.Throughput()), fmt.Sprintf("%f", c.Duration().Seconds())}
...
func (s *Sample) String() string {
	...
	return fmt.Sprintf("%s msgs/sec ~ %s/sec duaration %v sec, msgs %d", rate, throughput, s.Duration().Seconds(), s.JobMsgCnt)
}
